  ...
  const addTileNote = useCallback(
    async (acceptedFiles: File[]) => {
      ...
      if (import.meta.env.OCRSPACE_API_KEY)
      {
        ...
        const ocrResponse = await ocrSpace(ocrSourceImage!, import.meta.env.OCRSPACE_API_KEY);
        let noteText = ocrResponse.ParsedResults[0].ParsedText.replaceAll(/^[\d]*\s*/gm, '').trim();
        if (mulanamode === 1)
        {
